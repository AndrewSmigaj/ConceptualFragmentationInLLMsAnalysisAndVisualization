#!/usr/bin/env python3
"""
Generate enhanced Sankey diagrams for k=10 clustering with:
- Better node positioning to avoid overlap
- Colored and labeled archetypal paths
- Path descriptions
"""

import json
import sys
from pathlib import Path
import plotly.graph_objects as go
from collections import defaultdict
import logging
import numpy as np

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# Add project root to path
root_dir = Path(__file__).parent.parent.parent.parent
if str(root_dir) not in sys.path:
    sys.path.insert(0, str(root_dir))


class EnhancedSankeyGenerator:
    """Generate enhanced Sankey diagrams with labeled archetypal paths."""
    
    def __init__(self, base_dir: Path, k: int = 10):
        self.base_dir = base_dir
        self.k = k
        self.results_dir = base_dir / f"k{k}_analysis_results"
        
        # Load analysis results
        windowed_path = self.results_dir / f"windowed_analysis_k{k}.json"
        with open(windowed_path, 'r') as f:
            self.windowed_results = json.load(f)
        
        # Load semantic labels
        labels_path = base_dir / f"llm_labels_k{k}" / f"cluster_labels_k{k}.json"
        with open(labels_path, 'r') as f:
            label_data = json.load(f)
            self.semantic_labels = label_data['labels']
        
        # Load semantic purity data
        purity_path = base_dir / f"llm_labels_k{k}" / f"semantic_purity_k{k}.json"
        if purity_path.exists():
            with open(purity_path, 'r') as f:
                self.purity_data = json.load(f)
        else:
            self.purity_data = {}
        
        # Define colors for top 25 paths
        self.path_colors = [
            'rgba(255, 99, 71, 0.8)',    # Tomato
            'rgba(30, 144, 255, 0.8)',   # Dodger Blue
            'rgba(50, 205, 50, 0.8)',    # Lime Green
            'rgba(255, 215, 0, 0.8)',    # Gold
            'rgba(138, 43, 226, 0.8)',   # Blue Violet
            'rgba(255, 140, 0, 0.8)',    # Dark Orange
            'rgba(0, 206, 209, 0.8)',    # Dark Turquoise
            'rgba(255, 20, 147, 0.8)',   # Deep Pink
            'rgba(154, 205, 50, 0.8)',   # Yellow Green
            'rgba(219, 112, 147, 0.8)',  # Pale Violet Red
            'rgba(100, 149, 237, 0.8)',  # Cornflower Blue
            'rgba(255, 182, 193, 0.8)',  # Light Pink
            'rgba(144, 238, 144, 0.8)',  # Light Green
            'rgba(255, 160, 122, 0.8)',  # Light Salmon
            'rgba(176, 196, 222, 0.8)',  # Light Steel Blue
            'rgba(220, 20, 60, 0.8)',    # Crimson
            'rgba(75, 0, 130, 0.8)',     # Indigo
            'rgba(255, 127, 80, 0.8)',   # Coral
            'rgba(0, 128, 128, 0.8)',    # Teal
            'rgba(240, 128, 128, 0.8)',  # Light Coral
            'rgba(32, 178, 170, 0.8)',   # Light Sea Green
            'rgba(250, 128, 114, 0.8)',  # Salmon
            'rgba(0, 191, 255, 0.8)',    # Deep Sky Blue
            'rgba(127, 255, 0, 0.8)',    # Chartreuse
            'rgba(255, 0, 255, 0.8)'     # Magenta
        ]
    
    def generate_path_description(self, semantic_labels):
        """Generate a concise description of a path based on its semantic transitions."""
        # Extract primary labels
        primary_labels = []
        for label in semantic_labels:
            if '(' in label:
                primary = label.split('(')[0].strip()
            else:
                primary = label
            primary_labels.append(primary)
        
        # Identify key transitions
        transitions = []
        for i in range(len(primary_labels) - 1):
            if primary_labels[i] != primary_labels[i+1]:
                transitions.append(f"{primary_labels[i]}→{primary_labels[i+1]}")
        
        # Create description based on path pattern
        if len(set(primary_labels)) == 1:
            return f"Pure {primary_labels[0]}"
        elif "Function Words" in primary_labels and "Content Words" in primary_labels:
            return "Function-Content Bridge"
        elif "Pronouns" in primary_labels and "Auxiliaries" in primary_labels:
            return "Grammar Integration"
        elif "Punctuation" in primary_labels:
            if primary_labels[0] == "Punctuation":
                return "Punctuation Origin"
            elif primary_labels[-1] == "Punctuation":
                return "Punctuation Convergence"
            else:
                return "Punctuation Transit"
        elif len(transitions) == 1:
            return f"{transitions[0]} Shift"
        else:
            # Use first and last for complex paths
            return f"{primary_labels[0]}→{primary_labels[-1]}"
    
    def create_enhanced_sankey(self, window_name: str, top_n: int = 25):
        """Create enhanced Sankey diagram with better layout and path labeling."""
        window_data = self.windowed_results[window_name]
        
        if 'archetypal_paths' not in window_data:
            logging.warning(f"No archetypal paths found for {window_name} window")
            return None
        
        # Get top N paths
        top_paths = window_data['archetypal_paths'][:top_n]
        
        # Extract layer range
        layer_map = {
            'early': [0, 1, 2, 3],
            'middle': [4, 5, 6, 7],
            'late': [8, 9, 10, 11]
        }
        layers = layer_map[window_name]
        
        # Build nodes with better positioning
        nodes = []
        node_labels = []
        node_colors = []
        node_map = {}  # (layer, cluster) -> node_index
        
        # Calculate node positions to avoid overlap
        node_x = []
        node_y = []
        
        # X positions for each layer (spread evenly)
        layer_x_positions = np.linspace(0.05, 0.95, len(layers))
        
        # Create nodes for each layer
        for layer_idx, layer in enumerate(layers):
            layer_clusters = set()
            # Find all clusters used in top paths at this layer
            for path_info in top_paths:
                path = path_info['path']
                if layer_idx < len(path):
                    layer_clusters.add(path[layer_idx])
            
            # Y positions for nodes in this layer (spread to avoid overlap)
            n_clusters = len(layer_clusters)
            if n_clusters > 0:
                # More spacing for last layer to avoid overlap
                if layer_idx == len(layers) - 1:
                    y_positions = np.linspace(0.1, 0.9, n_clusters)
                else:
                    y_positions = np.linspace(0.15, 0.85, n_clusters)
            else:
                y_positions = []
            
            # Create nodes for this layer
            for cluster_idx, cluster in enumerate(sorted(layer_clusters)):
                node_idx = len(nodes)
                cluster_key = f"L{layer}_C{cluster}"
                layer_key = f"layer_{layer}"
                
                # Get semantic label with purity
                if layer_key in self.semantic_labels and cluster_key in self.semantic_labels[layer_key]:
                    label_info = self.semantic_labels[layer_key][cluster_key]
                    base_label = label_info['label']
                    
                    # Add purity percentage if available
                    if (layer_key in self.purity_data and 
                        cluster_key in self.purity_data[layer_key]):
                        purity = self.purity_data[layer_key][cluster_key]['purity']
                        label = f"{base_label}<br>({purity:.0f}%)" 
                    else:
                        label = base_label
                else:
                    label = f"C{cluster}"
                
                node_labels.append(label)
                node_colors.append('rgba(150, 150, 150, 0.8)')
                node_x.append(layer_x_positions[layer_idx])
                node_y.append(y_positions[cluster_idx] if cluster_idx < len(y_positions) else 0.5)
                
                node_map[(layer, cluster)] = node_idx
        
        # Build links from paths with colors
        links = []
        link_colors = []
        path_link_map = defaultdict(list)  # Track which links belong to which path
        
        # Process each path
        for path_idx, path_info in enumerate(top_paths):
            path = path_info['path']
            frequency = path_info['frequency']
            path_color = self.path_colors[path_idx % len(self.path_colors)]
            
            # Track links for this path
            for i in range(len(path) - 1):
                source_layer = layers[i]
                target_layer = layers[i + 1]
                source_cluster = path[i]
                target_cluster = path[i + 1]
                
                source_idx = node_map.get((source_layer, source_cluster))
                target_idx = node_map.get((target_layer, target_cluster))
                
                if source_idx is not None and target_idx is not None:
                    link_key = (source_idx, target_idx)
                    path_link_map[link_key].append((path_idx, frequency))
        
        # Create links with appropriate colors
        link_values = defaultdict(int)
        link_path_colors = {}
        
        for link_key, path_list in path_link_map.items():
            # Sum frequencies for this link
            total_freq = sum(freq for _, freq in path_list)
            link_values[link_key] = total_freq
            
            # Use color of the highest frequency path for this link
            dominant_path = max(path_list, key=lambda x: x[1])[0]
            link_path_colors[link_key] = self.path_colors[dominant_path % len(self.path_colors)]
        
        # Convert to Sankey format
        for (source, target), value in link_values.items():
            links.append({
                'source': source,
                'target': target,
                'value': value,
                'color': link_path_colors.get((source, target), 'rgba(0, 0, 0, 0.2)')
            })
        
        # Create Sankey trace
        fig = go.Figure(data=[go.Sankey(
            arrangement='snap',  # Better automatic arrangement
            node=dict(
                pad=25,  # More padding between nodes
                thickness=20,
                line=dict(color="black", width=0.5),
                label=node_labels,
                color=node_colors,
                x=node_x,  # Explicit x positions
                y=node_y   # Explicit y positions
            ),
            link=dict(
                source=[l['source'] for l in links],
                target=[l['target'] for l in links],
                value=[l['value'] for l in links],
                color=[l['color'] for l in links]
            )
        )])
        
        # Add annotations for path descriptions
        annotations = []
        
        # Add layer labels at the top
        for i, layer in enumerate(layers):
            annotations.append(dict(
                x=layer_x_positions[i],
                y=1.05,
                text=f"<b>Layer {layer}</b>",
                showarrow=False,
                xref='x',
                yref='paper',
                xanchor='center',
                font=dict(size=14)
            ))
        
        # Add path legend on the right
        legend_y_start = 0.95
        for i, path_info in enumerate(top_paths):
            # Generate path description
            path_desc = self.generate_path_description(path_info['semantic_labels'])
            frequency = path_info['frequency']
            
            # Create legend entry
            annotations.append(dict(
                x=1.02,
                y=legend_y_start - i * 0.035,  # Smaller spacing for 25 paths
                text=f"<b>Path {i+1}:</b> {path_desc} ({frequency} tokens)",
                showarrow=False,
                xref='paper',
                yref='paper',
                xanchor='left',
                font=dict(
                    size=9,  # Slightly smaller font
                    color=self.path_colors[i % len(self.path_colors)]
                )
            ))
        
        # Update layout
        fig.update_layout(
            title={
                'text': f"K={self.k} Token Flow - {window_name.capitalize()} Window (Top {top_n} Archetypal Paths)",
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 18}
            },
            font_size=12,
            height=800,
            width=1400,  # Wider to accommodate legend
            margin=dict(l=50, r=250, t=100, b=50),  # More right margin for legend
            annotations=annotations
        )
        
        return fig
    
    def generate_all_enhanced_sankeys(self, top_n=25):
        """Generate enhanced Sankey diagrams for all windows."""
        for window in ['early', 'middle', 'late']:
            logging.info(f"Generating enhanced Sankey for {window} window...")
            
            fig = self.create_enhanced_sankey(window, top_n=top_n)
            if fig:
                # Save HTML
                output_path = self.results_dir / f"sankey_{window}_k{self.k}_enhanced.html"
                fig.write_html(str(output_path))
                logging.info(f"Saved enhanced {window} Sankey to {output_path}")
                
                # Also save static image if kaleido is installed
                try:
                    img_path = self.results_dir / f"sankey_{window}_k{self.k}_enhanced.png"
                    fig.write_image(str(img_path), width=1400, height=800, scale=2)
                    logging.info(f"Saved static image to {img_path}")
                except:
                    pass


def main():
    """Generate enhanced Sankey diagrams."""
    base_dir = Path(__file__).parent
    
    print("============================================================")
    print("GENERATING ENHANCED K=10 SANKEY DIAGRAMS")
    print("============================================================")
    
    generator = EnhancedSankeyGenerator(base_dir, k=10)
    generator.generate_all_enhanced_sankeys()
    
    print("\n============================================================")
    print("ENHANCED SANKEY GENERATION COMPLETE")
    print("============================================================")
    print(f"\nGenerated outputs in {generator.results_dir}:")
    print(f"  - sankey_early_k10_enhanced.html")
    print(f"  - sankey_middle_k10_enhanced.html")
    print(f"  - sankey_late_k10_enhanced.html")


if __name__ == "__main__":
    main()