...
class AllTokensSankeyGenerator:
    ...
    def create_structured_sankey(self, window_name: str, window_data: dict, 
        ...
        fig.update_layout(
            title={
                'text': f"GPT-2 All Tokens {window_info['name']}: Top {len(paths)} Archetypal Paths",
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 20}
            },
    ...
    def generate_all_sankeys(self):
        ...
        for window_name, window_data in self.windows.items():
            ...
            if fig:
                # Save as HTML
                html_path = self.output_dir / f"all_tokens_sankey_{window_name}.html"
                fig.write_html(str(html_path))
                logging.info(f"  Saved HTML: {html_path}")
                
                # Save as PNG if kaleido is installed
                try:
                    png_path = self.output_dir / f"all_tokens_sankey_{window_name}.png"
...
<body>
    <h1>GPT-2 All Tokens (50,257) Trajectory Analysis</h1>
    <div class="stats">
        <h3>Overall Statistics</h3>
        <p><strong>Total Tokens:</strong> 50,257</p>
...
"""
    ...
    <div class="window-container">
        ...
        <iframe src="all_tokens_sankey_{window_name}.html"></iframe>
    </div>
"""
...
"""
        ...
        combined_path = self.output_dir / "all_tokens_sankey_combined.html"
...
def main():
    ...
    generator = AllTokensSankeyGenerator(base_dir)
