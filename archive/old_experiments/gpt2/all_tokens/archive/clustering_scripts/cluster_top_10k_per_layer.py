#!/usr/bin/env python3
"""
Find optimal k for each layer of top 10k GPT-2 token representations using gap statistic
and morphological/semantic purity metrics.

This version works with only the top 10k most common tokens for faster analysis.
"""
...
class PerLayerClusterAnalyzer:
    def __init__(self, base_dir: Path):
        ...
        # Load top 10k tokens
        logging.info("Loading top 10k token analysis...")
        
        # First check if top 10k data exists
        top_10k_path = base_dir / "top_10k_tokens_full.json"
        if not top_10k_path.exists():
            logging.info("Top 10k data not found. Extracting from full token list...")
            # Run extraction
            from extract_top_10k_tokens import extract_top_10k_tokens
            extract_top_10k_tokens()
        
        with open(top_10k_path, 'r', encoding='utf-8') as f:
            token_list = json.load(f)
        
        # Load top 10k token IDs for indexing
        with open(base_dir / "top_10k_token_ids.json", 'r') as f:
            self.top_10k_ids = json.load(f)
        ...
        logging.info(f"Loaded info for {len(self.token_info)} tokens (top 10k)")
        logging.info(f"Token ID range: {min(self.top_10k_ids)} - {max(self.top_10k_ids)}")
    ...
    def load_all_activations(self) -> np.ndarray:
        """Load top 10k activations."""
        logging.info("Loading top 10k activations...")
        
        # Check if pre-extracted file exists
        top_10k_activations_path = self.base_dir / "top_10k_activations.npy"
        
        if top_10k_activations_path.exists():
            activations = np.load(top_10k_activations_path)
            logging.info(f"Loaded pre-extracted activations shape: {activations.shape}")
        else:
            # Extract on the fly
            logging.info("Extracting top 10k activations from chunks...")
            all_activations = []
            chunk_files = sorted((self.base_dir / "activations").glob("activations_chunk_*.npy"))
            
            # Create mapping of token_id to position in top 10k
            id_to_position = {tid: i for i, tid in enumerate(self.top_10k_ids)}
            ...
            for chunk_file in tqdm(chunk_files, desc="Loading chunks"):
                ...
                # Find which top 10k tokens are in this chunk
                for local_idx in range(chunk_size):
                    global_idx = tokens_processed + local_idx
                    if global_idx in id_to_position:
                        all_activations.append((id_to_position[global_idx], chunk[local_idx]))
                
                tokens_processed += chunk_size
            
            # Sort by position and extract activations
            all_activations.sort(key=lambda x: x[0])
            activations = np.stack([act for _, act in all_activations])
            
            # Save for next time
            np.save(top_10k_activations_path, activations)
            logging.info(f"Saved top 10k activations shape: {activations.shape}")
    ...
    def run_per_layer_analysis_adaptive(self, layers_to_analyze: List[int] = None, 
        ...
        # Save results
        with open(self.results_dir / "per_layer_adaptive_results_top10k.json", 'w') as f:
    ...
    def generate_report(self, results: Dict):
        ...
        report_lines = [
            "# GPT-2 Top 10k Tokens Per-Layer Optimal Clustering Analysis",
            f"\nAnalyzed top {results['total_tokens']:,} most common tokens across {results['num_layers']} layers",
        ...
        with open(self.results_dir / "per_layer_report_top10k.md", 'w') as f:
            f.write('\n'.join(report_lines))
...
def main():
    ...
    # For 10k tokens, we can use a smaller k_max for faster analysis
    results = analyzer.run_per_layer_analysis_adaptive(k_min=2, k_max=300)
