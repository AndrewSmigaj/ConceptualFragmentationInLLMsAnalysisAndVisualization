...
class PerLayerClusterAnalyzer:
    ...
    def find_optimal_k_adaptive(self, layer_activations: np.ndarray, layer_idx: int, 
        ...
        """
        Adaptive search for optimal k using gap statistic elbow method.
        
        Strategy:
        1. Coarse exponential sampling to find the region
        2. Binary search for elbow point  
        3. Fine-grained search around elbow
        """
        import time
        logging.info(f"\nFinding optimal k for layer {layer_idx} using adaptive search...")
        
        # Phase 1: Exponential sampling to find rough region
        k_values = []
        k = k_min
        while k <= k_max:
            k_values.append(k)
            k = int(k * 1.5)  # Exponential growth
        if k_values[-1] < k_max:
            k_values.append(k_max)
            
        logging.info(f"Phase 1: Testing {len(k_values)} k values: {k_values}")
        
        # Calculate gap statistics for coarse grid
        gap_scores = {}
        gap_stds = {}
        silhouette_scores = {}
        all_labels = {}
        ...
        for i, k in enumerate(tqdm(k_values, desc="Coarse search")):
            k_start = time.time()
            ...
            if i > 0:
                ...
                remaining_ks = len(k_values) - i
                eta = remaining_ks * avg_time_per_k
                logging.info(f"  Testing k={k} ({i+1}/{len(k_values)}) - ETA: {eta/60:.1f} min")
            gap, std = self.calculate_gap_statistic(layer_activations, k, n_refs=5)
            gap_scores[k] = gap
            gap_stds[k] = std
            
            # Also get labels and silhouette for potential use
        ...
        # Find elbow using gap statistic criterion
        elbow_k = None
        for i in range(len(k_values) - 1):
            k_curr = k_values[i]
            k_next = k_values[i + 1]
            if gap_scores[k_curr] >= gap_scores[k_next] - gap_stds[k_next]:
                elbow_k = k_curr
                logging.info(f"Elbow detected at k={elbow_k}")
                break
        
        if elbow_k is None:
            # If no elbow found, use k with highest gap score
            elbow_k = max(gap_scores.keys(), key=lambda k: gap_scores[k])
            logging.info(f"No clear elbow, using k with highest gap: {elbow_k}")
        
        # Phase 2: Refine around elbow
        k_refined_min = max(k_min, int(elbow_k // 1.5))
        k_refined_max = min(k_max, int(elbow_k * 1.5))
        
        logging.info(f"\nPhase 2: Refining search in range [{k_refined_min}, {k_refined_max}]")
        
        # Fine-grained search with full computation
        best_k = elbow_k
        best_combined_score = -float('inf')
        detailed_results = {}
        
        # Determine step size (at least 20 points in refined range)
        step = max(1, (k_refined_max - k_refined_min) // 20)
        
        for k in tqdm(range(k_refined_min, k_refined_max + 1, step), desc="Fine search"):
            ...
            # Combined score
            combined = (0.5 * gap_scores[k] + 
                       0.3 * silhouette_scores[k] + 
        ...
        logging.info(f"Optimal k for layer {layer_idx}: {best_k} (combined score: {best_combined_score:.4f})")
        ...
        results = {
            ...
            'refined_range': [k_refined_min, k_refined_max],
            'elbow_k': elbow_k,
