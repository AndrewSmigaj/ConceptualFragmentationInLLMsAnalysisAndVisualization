#!/usr/bin/env python3
"""
Generate ALL GPT-2 figures for the paper with proper labels and colors.
This includes:
1. Sankey diagrams with LLM cluster labels
2. Stepped layer visualizations with colored paths
...
# Colors for grammatical categories (matching paper's color scheme)
GRAMMATICAL_COLORS = {
    "noun": "#1f77b4",      # Blue
    "verb": "#d62728",      # Red  
...
def load_expanded_data():
    ...
    return analysis_results, word_to_type

def generate_sankey_diagram(window_name, analysis_results, output_dir):
    ...
    for path_str, count in path_distribution.items():
        ...
        if "Entity" in path_str or "Noun" in path_str or "_C1" in path_parts[-1]:
            color = f"rgba(31, 119, 180, 0.5)"  # Blue with transparency
        elif "Modifier" in path_str or "Property" in path_str or "_C0" in path_parts[-1]:
...
def generate_stepped_visualization(window_name, analysis_results, output_dir):
    """Generate stepped layer visualization with colored paths and labels."""
    ...
    fig.update_layout(
        title={
            'text': f"GPT-2 {window_info['title']}<br>" +
                   f"<sub>Top 5 paths from {path_data['unique_paths']} unique trajectories</sub>",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 20}
        },
...
def main():
    ...
    # Load data
    analysis_results, word_to_type = load_expanded_data()
    ...
    for window_name in ["early", "middle", "late"]:
        ...
        generate_sankey_diagram(window_name, analysis_results, output_dir)
        
        # Generate stepped visualization
        generate_stepped_visualization(window_name, analysis_results, output_dir)
